...
/**
 * ./bin/run-example ml.OraDecisionTree  [options]
 ...
 * For local mode, run
 * ./bin/spark-submit --class DecisionTree  --driver-memory 1g
 *   [examples JAR path] [options]
 * If you use it as a template to create your own app, please use `spark-submit` to submit your app.
 */

object OraDecisionTree  {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val parser = new OptionParser[Params]("DecisionTree") {
      head("DecisionTree: an example decision tree app.")
      opt[String]("algo")
  ...
  private[ml] def loadDatasets(
    ...
    val origs: DataFrame = loadData(spark, input, dataFormat)


    val dataframes: Array[DataFrame] = if (testInput != "") {

      val numFeatures = origs.first().getAs[Vector](1).size
      val origTests: DataFrame =
        loadData(spark, testInput, dataFormat, Some(numFeatures))
      Array(origs, origTests)
    } else {

      origs.randomSplit(Array(1.0 - fracTest, fracTest), seed = 12345)
    }
